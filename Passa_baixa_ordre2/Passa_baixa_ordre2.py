...
img = Image.open('Passa_baixa_ordre2.PNG')
img.show()
...
while(repetir==True):
    ...
    with open("Passa_baixa_ordre2.PNG", "rb") as imageFile:
        foto = base64.b64encode(imageFile.read())
        fotostr=foto.decode("utf-8")

    #Creació del fitxer XML
    if numrep==0:
        with open("Passa_baixa_ordre2.xml","a") as file:
            ...
            file.write("<question type=\"category\">\n")
            file.write("<category>\n")
            file.write("<text>$course$/top/PythonQuiz/Analogic/Filtres/Passa baixa ordre 2</text>\n")
            file.write("</category>\n")
            file.write("</question>\n")

    for i in range (permutacions):
        #Generació dels valors de les resistències
        C2=r.randrange(2,7,1)
        ...
        funciodolenta3='H(s)=A'+'/'+'('+str(C2)+'(sRC)^2+'+str(2*C2+1)+'sRC+1)'
        funciodolenta4='H(s)=A'+'/'+'('+str(C2)+'(sRC)^2+('+str(2*C2+2)+'-A)sRC+1)'

        #Generació dels tipus de filtres
        tipusfiltrebo="Passa baixa d'ordre 2"
        ...
        enunciat_xml="El circuit de la figura és un filtre actiu amb arquitectura de Sallen-Key simplificada, que presenta una funció de transferencia H(s) com la indicada al costat de la figura. Les admitàncies Y2 i Y4 corresponen a dos condensadors de capacitat C i "+str(C2)+"C respectivament. Les admitàncies Y1 i Y3 corresponen a dues resistències de valor R. El valor del guany de l'amplificador és A." 
        if tipus==3:
        ...
        #Es crea l'enunciat del fitxer XML
        enunciat_py="El valor del condensador és C2: "+str(C2)+"C\n"
        if tipus==3:
        ...
        with open("Passa_baixa_ordre2.xml","a") as file:
            ...
            file.write("<file name=\"Passa_baixa_ordre2.PNG\" path=\"/\" encoding=\"base64\">"+str(fotostr)+"\n"+"</file>\n")
            file.write("<text><![CDATA["+enunciat_xml+".<br><br><img src=\"@@PLUGINFILE@@/Passa_baixa_ordre2.PNG\" alt="" width=\"284\" height=\"425\" role=\"presentation\" class=\"img-responsive atto_image_button_text-bottom\"><br>]]></text>\n")
    ...
    print("Si vol generar un altre exercici de filtres passa baixa d'ordre 2, premi 0. En cas contrari, premi 1\n")
    while(repeticio<0 or repeticio>1):
        ...
        if repeticio0.isdigit()==False:
            ...
            if repeticio<0 or repeticio>1:
                ...
                repetir=False
                print("\nGràcies per haver utilitzat el generador d'exercicis de filtres passa baixa d'ordre 2!")
                with open("Passa_baixa_ordre2.xml","a") as file:
                    file.write("</quiz>")
            else:
                print("\n")
