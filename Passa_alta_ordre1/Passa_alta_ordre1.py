...
img = Image.open('Passa_alta_ordre1.PNG')
img.show()
...
while(repetir==True):
    ...
    with open("Passa_alta_ordre1.PNG", "rb") as imageFile:
        foto = base64.b64encode(imageFile.read())
        fotostr=foto.decode("utf-8")

    #Creació del fitxer XML
    if numrep==0:
        with open("Passa_alta_ordre1.xml","a") as file:
            ...
            file.write("<question type=\"category\">\n")
            file.write("<category>\n")
            file.write("<text>$course$/top/PythonQuiz/Analogic/Filtres/Passa alta ordre 1</text>\n")
            file.write("</category>\n")
            file.write("</question>\n")
    ...
    for i in range (permutacions):
        ...
        funciobona='H(s)='+str(int(-R*C1))+'sRC/'+'(1+'+str(int(R*C2))+'sRC)'
        funciodolenta1='H(s)='+str(int(R*C1))+'sRC/'+'(1+'+str(int(R*C2))+'sRC)'
        funciodolenta2='H(s)='+str(int(-R*C2))+'sRC/'+'(1+'+str(int(R*C2))+'sRC)'
        funciodolenta3='H(s)='+str(int(-R*C2))+'sRC/'+'(1+'+str(int(R*C1))+'sRC)'
        funciodolenta4='H(s)='+str(int(-R*C1))+'sRC/'+'(1+'+str(int(R*C1))+'sRC)'

        #Generació dels tipus de filtres
        tipusfiltrebo="Passa alta d'ordre 1"
        ...
        frequenciabona=str(round(1/(C2*R),3))+'/RC'
        frequenciadolenta1=str(round(C1/(C2*R),3))+'/RC'
        frequenciadolenta2=str(round(1/(R*C1),3))+'/RC'
        frequenciadolenta3=str(round(1/(C2*C1),3))+'/RC'
        frequenciadolenta4=str(round(1/(R*C1*C2),3))+'/RC'
        ...
        enunciat_xml="El circuit de la figura és un filtre basat en un amplificador operacional ideal. El valor de la resistència és de "+str(R)+'R i els valors del condensadors C1 i C2 són de '+str(C1)+'C i '+str(C2)+'C respectivament.'
        if tipus==0:
        ...
        enunciat_py="El valor de la resistència és de "+str(R)+'R. Els valors dels condensadors C1 i C2 són de '+str(C1)+'C i '+str(C2)+'C respectivament.\n'
        if tipus==3:
        ...
        with open("Passa_alta_ordre1.xml","a") as file:
            ...
            file.write("<file name=\"Passa_alta_ordre1.PNG\" path=\"/\" encoding=\"base64\">"+str(fotostr)+"\n"+"</file>\n")
            file.write("<text><![CDATA["+enunciat_xml+".<br><br><img src=\"@@PLUGINFILE@@/Passa_alta_ordre1.PNG\" alt="" width=\"284\" height=\"425\" role=\"presentation\" class=\"img-responsive atto_image_button_text-bottom\"><br>]]></text>\n")
    ...
    print("Si vol generar un altre exercici de filtres passa alta d'ordre 1, premi 0. En cas contrari, premi 1\n")
